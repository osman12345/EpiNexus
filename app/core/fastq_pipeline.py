...
class FASTQPipeline:
    ...
    def _run_fastqc(self, sample: SampleInfo) -> str:
        ...
        cmd = ["fastqc", "-o", str(self.dirs["fastqc"]), "-t", str(self.config.threads), sample.fastq_r1]
        if sample.fastq_r2:
    ...
    def _trim_reads(self, sample: SampleInfo) -> Tuple[str, Optional[str]]:
        ...
        if self.config.paired_end and sample.fastq_r2:
            ...
            cmd = [
                ...
                "--paired",
                "--quality",
                str(self.config.quality_threshold),
                "--length",
                str(self.config.min_length),
                "--cores",
                str(self.config.threads),
                "-o",
                str(self.dirs["trimmed"]),
                sample.fastq_r1,
                sample.fastq_r2,
            ]
        else:
        ...
        if self.config.aligner == "bowtie2":
            if r2:
                cmd = [
                    ...
                    str(self.config.threads),
                    "--maxins",
                    str(self.config.max_fragment_size),
                    "--no-mixed",
                    "--no-discordant",
                ]
            else:
                cmd = ["bowtie2", "-x", genome_idx, "-U", r1, "-p", str(self.config.threads)]
    ...
    def _filter_bam(self, sample: SampleInfo, bam_file: str) -> str:
        ...
        cmd = ["samtools", "view", "-b", "-q", str(self.config.min_mapq), "-o", mapq_bam, dedup_bam]
        ...
        if self.config.remove_blacklist:
            ...
            if blacklist and os.path.exists(blacklist):
                cmd = ["bedtools", "intersect", "-v", "-abam", mapq_bam, "-b", blacklist, "-wa"]
    ...
    def _call_peaks(self, sample: SampleInfo, bam_file: str, control_bam: str = None) -> str:
        ...
        if self.config.peak_caller == "macs2":
            cmd = [
                ...
                str(self.dirs["peaks"]),
                "-g",
                "hs" if "hg" in self.config.genome else "mm",
                "-q",
                str(self.config.qvalue_threshold),
            ]
    ...
    def _collect_stats(self, sample: SampleInfo, output: PipelineOutput) -> Dict:
        ...
        if output.bam_filtered and os.path.exists(output.bam_filtered):
            result = subprocess.run(["samtools", "flagstat", output.bam_filtered], capture_output=True, text=True)
    ...
    def _execute_piped(self, cmd1: List[str], cmd2: List[str], cmd3: List[str], description: str):
