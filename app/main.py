...
from .models.database import init_db, Job, Sample, Comparison
...
async def upload_sample_sheet(file: UploadFile = File(...), db: Session = Depends(get_db)):
...
@app.get("/comparisons")
async def list_comparisons(histone_mark: str = None, db: Session = Depends(get_db)):
...
async def create_job(job: JobCreate, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    db_job = Job(name=job.name, job_type=JobType(job.job_type.value), status=DBJobStatus.PENDING, config=job.config)
    db.add(db_job)
    db.commit()
    db.refresh(db_job)
...
async def run_diffbind_analysis(
    config: DiffBindConfig, background_tasks: BackgroundTasks, db: Session = Depends(get_db)
):
    """Submit a DiffBind differential analysis job."""
    from .models.database import JobType, JobStatus as DBJobStatus

    # Verify comparison exists
    comparison = db.query(Comparison).filter(Comparison.id == config.comparison_id).first()
    if not comparison:
        raise HTTPException(status_code=404, detail="Comparison not found")
...
async def run_integration_analysis(
    config: IntegrationConfig, background_tasks: BackgroundTasks, db: Session = Depends(get_db)
):
    ...
    for mark, comp_id in config.mark_comparisons.items():
        ...
        if not comparison:
            raise HTTPException(status_code=404, detail=f"Comparison not found for {mark}: {comp_id}")
...
async def get_results(job_id: str, db: Session = Depends(get_db)):
    ...
    return {"job_id": job.id, "job_type": job.job_type.value, "results": job.results, "output_dir": job.output_dir}


@app.get("/results/{job_id}/download/{filename}")
async def download_result_file(job_id: str, filename: str, db: Session = Depends(get_db)):
