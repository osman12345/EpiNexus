...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["🧬 Gene Annotation", "🔗 Pathway Analysis", "📍 Regulatory Elements", "🧪 Functional Enrichment"]
    )
...
def render_gene_annotation(peaks):
    ...
    with col1:
        ...
        annotation_db = st.selectbox(
            "Gene annotation database", ["GENCODE v43", "RefSeq", "UCSC Known Genes", "Ensembl"]
        )
        ...
        if st.button("Run Annotation", type="primary"):
            ...
            if HAS_WORKFLOW_MANAGER:
                WorkflowManager.record_step(
                    ...
                    parameters={"tss_distance": tss_distance * 1000, "n_peaks": len(peaks)},
                    inputs=["peaks"],
...
def render_pathway_analysis(peaks):
    ...
    with col1:
        ...
        background = st.selectbox("Background genes", ["All genes", "Expressed genes", "Custom list"])

        method = st.selectbox("Statistical method", ["Hypergeometric", "Fisher's exact", "GSEA"])
        ...
        if st.button("Run Pathway Analysis", type="primary"):
            ...
            if HAS_WORKFLOW_MANAGER:
                WorkflowManager.record_step(
                    ...
                    parameters={"gene_set": gene_set, "background": background, "method": method},
    ...
    with col2:
        ...
        pathways = pd.DataFrame(
            {
                ...
                "Overlap": [
                    f"{min(45, n_genes // 4)}/200",
                    f"{min(38, n_genes // 5)}/200",
                    f"{min(32, n_genes // 6)}/200",
                    f"{min(28, n_genes // 7)}/200",
                    f"{min(25, n_genes // 8)}/200",
                    f"{min(22, n_genes // 9)}/200",
                    f"{min(20, n_genes // 10)}/200",
                    f"{min(18, n_genes // 11)}/200",
                ],
        ...
        fig = px.bar(
            pathways,
            x=-np.log10(pathways["FDR"]),
            y="Pathway",
            orientation="h",
            color=-np.log10(pathways["FDR"]),
            color_continuous_scale="Reds",
        )
        fig.update_layout(xaxis_title="-Log10 FDR", yaxis_title="", height=400, showlegend=False)
...
def render_regulatory_elements(peaks):
    ...
    with col2:
        ...
        if "Distance_TSS" in annotated.columns:
            ...
            fig.update_layout(xaxis_title="Distance to nearest TSS (bp)", yaxis_title="Peak count", height=400)
    ...
    if "Feature" in annotated.columns:
        feature_stats = annotated.groupby("Feature").agg({"start": "count", "Distance_TSS": "mean"}).round(0)
...
def render_functional_enrichment(peaks):
    ...
    enrichment_type = st.selectbox(
        "Enrichment type", ["TF Binding Sites", "Disease Associations", "Drug Targets", "Protein Domains"]
    )
    ...
    if enrichment_type == "TF Binding Sites":
        ...
        with col1:
            fig = px.bar(
                tf_results, x="Enrichment", y="TF", orientation="h", color="Enrichment", color_continuous_scale="Reds"
            )
        ...
        fig = px.bar(
            disease_results, x="Disease", y="Enrichment", color="Associated Genes", color_continuous_scale="Blues"
        )
        fig.update_layout(height=400)
        st.plotly_chart(fig, use_container_width=True)
