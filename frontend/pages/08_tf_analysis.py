...
st.set_page_config(page_title="TF Analysis - EpiNexus", page_icon="🧬", layout="wide")
...
def main():
    ...
    with st.sidebar:
        ...
        analysis_type = st.selectbox(
            "Analysis Type",
            ["Motif Enrichment", "TF Binding Site Scan", "TF-Target Prediction", "TF + Histone Integration"],
        )
...
def render_motif_enrichment():
    ...
    with col2:
        ...
        bg_option = st.radio("Background selection", ["All peaks", "Upload custom", "Genome-wide (shuffled)"])

        if bg_option == "Upload custom":
            bg_file = st.file_uploader("Upload background peaks", type=["bed", "csv", "tsv"], key="bg_peaks")
    ...
    with col1:
        genome = st.selectbox("Select genome", ["mm10", "mm39", "hg38", "hg19", "rn6"])

    with col2:
        genome_fasta = st.text_input("Path to genome FASTA (optional)", placeholder="/path/to/genome.fa")

    # Motif database selection
    st.subheader("Motif Database")
    motif_source = st.radio(
        "Motif source", ["Built-in JASPAR (common TFs)", "Upload custom motifs", "Full JASPAR database"]
    )

    if motif_source == "Upload custom":
        motif_file = st.file_uploader("Upload motifs (JASPAR format)", type=["txt", "jaspar", "meme"])
...
def render_binding_site_scan():
    ...
    with col1:
        peaks_file = st.file_uploader("Upload peaks to scan", type=["bed", "csv"], key="scan_peaks")

    with col2:
        # TF selection
        st.subheader("Select TFs")
        tf_options = ["AP-1 (FOS/JUN)", "NF-κB", "GATA1/2", "ETS1", "MEF2A", "SP1", "CREB1", "TP53", "All available"]
...
def render_target_prediction():
    ...
    with col1:
        ...
        tf_file = st.file_uploader(
            "Upload TF peaks (ChIP-seq/CUT&Tag)", type=["bed", "csv", "narrowPeak"], key="tf_peaks"
        )
    ...
    with col2:
        ...
        expr_file = st.file_uploader("Upload DE results", type=["csv", "tsv", "xlsx"], key="expr_data")

    st.subheader("Prediction Parameters")
    col1, col2, col3 = st.columns(3)

    with col1:
        max_distance = st.number_input("Max distance from TSS (bp)", value=100000, step=10000)

    with col2:
        promoter_dist = st.number_input("Promoter region (bp)", value=3000, step=500)

    with col3:
        min_confidence = st.selectbox("Minimum confidence", ["All", "Low", "Medium", "High"])
        ...
        with st.spinner("Predicting TF targets..."):
            ...
            if tf_file:
                if tf_file.name.endswith(".csv"):
                    ...
                    tf_peaks_df.columns = ["chr", "start", "end"] + [
                        f"col_{i}" for i in range(3, len(tf_peaks_df.columns))
                    ]
            ...
            if expr_file:
                expr_df = (
                    pd.read_csv(expr_file) if expr_file.name.endswith(".csv") else pd.read_csv(expr_file, sep="\t")
                )
...
def render_tf_histone_integration():
    ...
    with col1:
        ...
        tf_file = st.file_uploader("TF binding peaks", type=["bed", "csv"], key="tf_int")
    ...
    st.subheader("Expression Data (Optional)")
    expr_file = st.file_uploader("Gene expression DE results", type=["csv", "tsv"], key="expr_int")
...
def run_motif_enrichment(
    target_peaks: pd.DataFrame, background_peaks: pd.DataFrame = None, tf_name: str = None
) -> pd.DataFrame:
    """
...
def integrate_tf_histone_data(
    tf_peaks: pd.DataFrame = None,
...
def display_enrichment_results(results: pd.DataFrame):
    ...
    st.download_button("📥 Download Results (CSV)", csv, "motif_enrichment_results.csv", "text/csv")
...
def display_target_predictions(results: pd.DataFrame):
    ...
    fig = px.scatter(
        ...
        labels={"Distance_to_TSS": "Distance to TSS (bp)", "Expression_log2FC": "Expression Log2 Fold Change"},
