...
st.set_page_config(page_title="ATAC-seq Analysis - EpiNexus", page_icon="🔓", layout="wide")
...
def render_qc_metrics(peaks):
    ...
    with col3:
        st.metric("TSS Enrichment (est.)", f"{tss_enrichment:.1f}", delta="Good" if tss_enrichment > 5 else "Low")
    ...
    with col2:
        ...
        size_cats = pd.cut(
            ...
            labels=["NFR (<100)", "Narrow (100-200)", "Medium (200-500)", "Wide (500-1kb)", "Very Wide (>1kb)"],
        )
...
def render_fragment_analysis(peaks):
    ...
    with col1:
        ...
        fig.add_trace(go.Histogram(x=widths, nbinsx=100, name="Peak Widths", marker_color="#3498db"))

        # Add region annotations
        fig.add_vrect(x0=0, x1=100, fillcolor="#27ae60", opacity=0.2, annotation_text="NFR", annotation_position="top")
        fig.add_vrect(x0=180, x1=247, fillcolor="#f39c12", opacity=0.2, annotation_text="Mono")
        fig.add_vrect(x0=315, x1=473, fillcolor="#9b59b6", opacity=0.2, annotation_text="Di")

        fig.update_layout(
            title="Peak Width Distribution", xaxis_title="Peak Width (bp)", yaxis_title="Count", height=400
        )
...
def render_differential(peaks):
    ...
    with col1:
        ...
        method = st.selectbox("Method", ["DESeq2", "edgeR", "Wilcoxon"])
        ...
        if st.button("Run Analysis", type="primary"):
            ...
            if HAS_WORKFLOW_MANAGER:
                WorkflowManager.record_step(
                    ...
                    parameters={"fdr_threshold": fdr_thresh, "min_fold_change": fc_thresh, "method": method},
                    inputs=["atac_peaks"],
    ...
    with col2:
        if not has_conditions:
            ...
            if signal_col:
                fig = px.histogram(peaks, x=signal_col, nbins=50, title=f"Peak Signal Distribution ({signal_col})")
        ...
        # Volcano plot
        diff_results["Significant"] = (diff_results["padj"] < fdr_thresh) & (
            np.abs(diff_results["log2FoldChange"]) > fc_thresh
        )

        diff_results["Direction"] = "Not Significant"
        diff_results.loc[diff_results["Significant"] & (diff_results["log2FoldChange"] > 0), "Direction"] = (
            "More Accessible"
        )
        diff_results.loc[diff_results["Significant"] & (diff_results["log2FoldChange"] < 0), "Direction"] = (
            "Less Accessible"
...
def render_footprinting(peaks):
    ...
    with col1:
        tf_select = st.selectbox("Select Transcription Factor", ["CTCF", "SP1", "NRF1", "REST", "GATA1", "MAX"])
    ...
    with col2:
        ...
        fig.add_vrect(x0=-10, x1=10, fillcolor="rgba(231, 76, 60, 0.3)", annotation_text=f"{tf_select} motif")
...
def render_histone_integration(peaks):
    ...
    def classify_peak(width):
        if width < 200:
            ...
            return np.random.choice(["Active Promoter", "Active Enhancer", "Poised Enhancer"], p=[0.5, 0.3, 0.2])
        elif width < 500:
            return np.random.choice(
                ["Active Enhancer", "Poised Enhancer", "Active Promoter", "Other Accessible"], p=[0.4, 0.25, 0.2, 0.15]
            )
        else:
            return np.random.choice(["Other Accessible", "Poised Enhancer", "Repressed"], p=[0.5, 0.3, 0.2])
    ...
    with col2:
        # State by peak width
        state_width = pd.DataFrame({"State": states, "Width": widths.values})

        fig = px.box(
            state_width, x="State", y="Width", color="State", color_discrete_sequence=px.colors.qualitative.Set2
        )
        fig.update_layout(title="Peak Width by Chromatin State", height=400, showlegend=False, xaxis_tickangle=-45)
        st.plotly_chart(fig, use_container_width=True)
