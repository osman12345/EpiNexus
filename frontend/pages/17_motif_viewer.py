...
st.set_page_config(page_title="Motif Viewer - EpiNexus", page_icon="🔤", layout="wide")
...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["📊 View Motifs", "🔀 Compare Motifs", "📤 Custom Motif"])
...
def render_motif_viewer():
    ...
    with col1:
        ...
        motif_name = st.selectbox("Transcription Factor", list_available_motifs())

        color_scheme = st.selectbox("Color Scheme", ["weblogo", "classic", "colorblind", "grayscale"])
        ...
        if motif:
            ...
            with st.expander("View Position Weight Matrix"):
                matrix_df = pd.DataFrame(
                    motif.matrix, index=["A", "C", "G", "T"], columns=[f"Pos {i + 1}" for i in range(motif.length)]
                )
...
def render_motif_comparison():
    ...
    with col1:
        motif1_name = st.selectbox("Motif 1", list_available_motifs(), key="compare_m1")

    with col2:
        motif2_name = st.selectbox("Motif 2", list_available_motifs(), index=1, key="compare_m2")
    ...
    if motif1 and motif2:
        ...
        with col1:
            ...
            st.metric("Pearson Correlation", f"{corr:.3f}", delta="Similar" if corr > 0.7 else "Different")

        with col2:
            st.metric("Euclidean Distance", f"{comparison['euclidean_distance']:.3f}")

        with col3:
            st.metric("KL Divergence", f"{comparison['kl_divergence']:.3f}")

        with col4:
            st.metric("Length Difference", f"{comparison['length_diff']} bp")
...
def render_custom_motif():
    ...
    input_method = st.radio("Input Method", ["Paste Matrix", "Upload File", "From Sequences"], horizontal=True)
    ...
    if input_method == "Paste Matrix":
        ...
        if st.button("Generate Logo"):
            try:
                ...
                if matrix.shape[0] != 4:
                    ...
                    custom_motif = MotifMatrix(name=motif_name, matrix=matrix, source="User input")
        ...
        uploaded = st.file_uploader("Upload motif file", type=["txt", "jaspar", "meme", "pfm"])
        ...
        sequences = st.text_area(
            "Enter aligned sequences (one per line)", value="CACGTG\nCACGTG\nCACGTG\nCATGTG\nCAGGTG", height=150
        )
        ...
        if st.button("Build Motif"):
            ...
            if len(seqs) < 2:
                ...
                custom_motif = MotifMatrix(
                    name="From Sequences", matrix=pfm, source=f"Built from {len(seqs)} sequences"
                )
                ...
                with st.expander("View Position Frequency Matrix"):
                    matrix_df = pd.DataFrame(
                        pfm, index=["A", "C", "G", "T"], columns=[f"Pos {i + 1}" for i in range(length)]
                    )
