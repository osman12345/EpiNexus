...
st.set_page_config(page_title="GWAS Overlap - EpiNexus", page_icon="🧬", layout="wide")
...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📤 Input Data", "📊 Enrichment Analysis", "🎯 Variant Prioritization", "📋 Disease Associations"]
    )
...
def render_input_data():
    ...
    with col1:
        ...
        peak_source = st.radio("Peak source", ["Use session data", "Upload new"], horizontal=True)
    ...
    with col2:
        ...
        gwas_source = st.selectbox("GWAS catalog source", ["Select traits", "Upload custom"])
...
def render_enrichment_analysis(peaks, variants):
    ...
    fig.add_vline(x=1, line_dash="dash", line_color="gray", annotation_text="No enrichment")

    fig.update_layout(xaxis_title="Fold Enrichment", yaxis_title="", height=500, showlegend=False)
...
def render_variant_prioritization(peaks, variants):
    ...
    if HAS_WORKFLOW_MANAGER:
        WorkflowManager.record_step(
            ...
            parameters={"n_variants": len(overlapping)},
    ...
    with col2:
        trait_filter = st.multiselect("Filter by trait", overlapping["trait"].unique().tolist())

    with col3:
        sort_by = st.selectbox("Sort by", ["priority_score", "pvalue", "peak_signal"])
    ...
    st.dataframe(filtered[available_cols].head(50).round(4), use_container_width=True, hide_index=True)
    ...
    fig.add_vline(x=min_score, line_dash="dash", line_color="red", annotation_text="Threshold")
    ...
    st.download_button("📥 Download Prioritized Variants", csv, "prioritized_variants.csv", "text/csv")
...
def render_disease_associations():
    ...
    associations = pd.DataFrame(
        {
            ...
            "Top Gene": ["TCF7L2", "CDKN2A", "BRCA1", "NOD2", "HLA-DRB1", "APOE", "IL33", "DRD2", "HLA-DRB1", "AR"],
        }
    )
    ...
    with col1:
        ...
        fig.update_layout(height=500, coloraxis_colorbar_title="-Log10 P-value")
    ...
    for trait in traits:
        try:
            ...
            response = requests.get(
                "https://www.ebi.ac.uk/gwas/rest/api/associations/search/findByDiseaseTrait",
...
def add_prioritization_scores(variants):
    ...
    variants["priority_score"] = np.clip((pvalue_score + signal_score) / 2, 0, 1)
