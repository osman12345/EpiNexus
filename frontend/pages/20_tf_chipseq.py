...
from app.core.tf_analysis import (
    TFAnalysisEngine, TFCoBindingAnalyzer, MotifEnrichmentAnalyzer,
    generate_demo_tf_peaks, generate_demo_genes
)

st.set_page_config(
    page_title="TF ChIP-seq - EpiNexus",
    page_icon="🔬",
    layout="wide"
)
...
def render_data_input():
    ...
    with col1:
        ...
        data_source = st.radio(
            "Data source",
            ["Demo data", "Upload peaks", "From preprocessing"],
            horizontal=True,
            key="tf_data_source"
        )
        ...
        if data_source == "Demo data":
            ...
            tf_name = st.selectbox(
                ...
                ["MYC", "P53", "CTCF", "STAT3", "NFkB", "Custom"]
            )
            ...
            uploaded = st.file_uploader(
                "Upload peak file (BED/narrowPeak/CSV)",
                type=['bed', 'narrowpeak', 'csv', 'tsv']
            )
            ...
            if uploaded:
                if uploaded.name.endswith('.csv'):
                    ...
                    if len(peaks.columns) >= 10:
                        peaks.columns = ['chr', 'start', 'end', 'peak_id', 'score',
                                        'strand', 'signal', 'pvalue', 'qvalue', 'summit'][:len(peaks.columns)]
                    else:
    ...
    with col2:
        ...
        if gene_source == "Demo genes":
            ...
            gene_file = st.file_uploader(
                "Upload gene annotation (BED/GTF/CSV)",
                type=['bed', 'gtf', 'csv', 'tsv']
            )
...
def render_peak_analysis(peaks: pd.DataFrame):
    ...
    with col1:
        ...
        fig = px.histogram(
            x=widths,
            nbins=50,
            title=f"{tf_name} Peak Width Distribution"
        )
        fig.update_layout(
        ...
        fig.add_vline(x=widths.median(), line_dash='dash', line_color='red',
                     annotation_text=f"Median: {widths.median():.0f} bp")
    ...
    with col2:
        ...
        if 'signal' in peaks.columns:
            fig = px.histogram(
                ...
                x='signal',
                nbins=50,
                title=f"{tf_name} Peak Signal Distribution"
            )
            fig.update_layout(
                xaxis_title="Signal (fold enrichment)",
                yaxis_title="Count",
                height=400
            )
            st.plotly_chart(fig, use_container_width=True)
...
def render_motif_analysis(peaks: pd.DataFrame):
    ...
    if st.session_state.get('tf_using_demo', True):
        st.info("🔬 **Demo Mode**: Showing simulated motif enrichment results. "
...
def render_target_analysis(peaks: pd.DataFrame, genes: pd.DataFrame):
    ...
    with col1:
        max_distance = st.slider(
            "Maximum distance from TSS (kb)",
            10, 500, 100
        ) * 1000

    with col2:
        promoter_def = st.slider(
            "Promoter definition (bp from TSS)",
            500, 5000, 2000
        )
    ...
    with col1:
        ...
        fig.update_layout(
            xaxis_title="Distance to TSS (kb)",
            yaxis_title="Count",
            height=400
        )
    ...
    with col2:
        ...
        fig = px.pie(
            ...
            title="Upstream vs Downstream Binding"
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    st.download_button(
        "📥 Download Target Genes",
        csv,
        f"{tf_name}_target_genes.csv",
        "text/csv"
    )
...
def render_cobinding_analysis():
    ...
    if results:
        ...
        with col1:
            ...
            values = [
                results[f'{tf1_name}_only'],
                results['overlapping_peaks'],
                results[f'{tf2_name}_only']
            ]
