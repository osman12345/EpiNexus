...
def run_alignment(fastq_r1: str, fastq_r2: str, output_bam: str,
                  aligner: str, index_path: str, threads: int = 4,
...
def render_single_alignment(tools):
    ...
    with col1:
        ...
        input_method = st.radio(
            "Input method",
            ["Upload FASTQ", "Specify file path"],
            horizontal=True
        )

        if input_method == "Upload FASTQ":
            fastq_r1 = st.file_uploader(
                "Read 1 (R1) FASTQ",
                type=['fastq', 'fq', 'gz'],
                key='fastq_r1'
            )
    ...
    with col2:
        ...
        aligner = st.selectbox(
            "Aligner",
            available_aligners,
            help="Bowtie2 recommended for ChIP-seq/CUT&Tag, BWA for ATAC-seq"
        )
        ...
        # Index path
        default_index = f"~/genomes/{genome}/bowtie2_index/{genome}" if aligner == 'bowtie2' else f"~/genomes/{genome}/bwa_index/{genome}"
        index_path = st.text_input(
            "Index path",
            value=default_index,
            help="Path to aligner index (without file extension)"
        )
        ...
        with st.expander("Advanced Options"):
            ...
            if aligner == 'bowtie2':
                preset = st.selectbox(
                    "Preset",
                    ["--sensitive", "--very-sensitive", "--fast", "--very-fast"],
                    index=1
                )
            ...
            if input_method == "Upload FASTQ":
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    result = run_alignment(
                        r1_path, r2_path, output_bam,
                        aligner, os.path.expanduser(index_path),
                        threads, extra_params
                    )
            else:
                status_text.text(f"Running {aligner} alignment...")
                progress_bar.progress(30)
...
def render_batch_alignment(tools):
    ...
    sample_sheet_method = st.radio(
        "Sample sheet input",
        ["Upload CSV", "Auto-detect from directory", "Manual entry"],
        horizontal=True
    )
    ...
    if sample_sheet_method == "Upload CSV":
        ...
        fastq_dir = st.text_input(
            "FASTQ directory",
            placeholder="/path/to/fastq_files/"
        )
        ...
        for i in range(num_samples):
            ...
            with col2:
                r1 = st.text_input(f"R1 path", key=f"batch_r1_{i}")
            with col3:
                r2 = st.text_input(f"R2 path (optional)", key=f"batch_r2_{i}")
...
def render_alignment_results():
    ...
    st.dataframe(
        results_df[['sample', 'bam', 'aligner', 'genome', 'mapping_rate']],
